...
public class SparePartServiceImpl implements SparePartService {
    ...
    public BaseResponseDTO addPart(String partName, String description, String manufacturer, Long price, String partNumber, List<MultipartFile> photos, Integer sGST, Integer cGST, Integer totalGST, Integer buyingPrice) {
        ...
        try {
            ...
            // Using the builder to create the entity WITH PHOTOS from the start
            SparePart sparePart = SparePart.builder()
                    ...
                    .totalGST(totalGST)
                    .photo(compressedPhotos) // Include photos in initial save
                    .build();

            // Save only once to avoid session issues
            sparePart = sparePartRepo.save(sparePart);

            logger.info("Saved new spare part with ID: {} and {} photos",
                    sparePart.getSparePartId(),
                    compressedPhotos.size());
            ...
            logger.error("Hibernate session error: {}", e.getMessage(), e);
            throw new BadRequestException("Database session conflict. Please try again.");
